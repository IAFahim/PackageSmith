...
static class Program
{
    ...
    {
        // 1. If no arguments, run the Interactive Main Menu
        if (args.Length == 0)
        {
            ShowBanner();
        ...
        // Only show banner if not running a specific command that needs clean output (like json)
        if (!args.Contains("--json") && !args.Contains("--quiet"))
        {
            ShowBanner();
        }
