...
public sealed class NewCommand : Command<NewCommand.Settings>
{
    ...
    {
        var selectedTemplate = settings.TemplateName;
        ...
        if (string.IsNullOrEmpty(selectedTemplate))
        {
            ...
            if (templates.Count > 0)
            {
                ...
                if (selection != "Standard (Empty)") selectedTemplate = selection;
            }
        }
    ...
    private int CreateFromTemplate(Settings settings, string templateName)
    {
        ...
        var packageName = settings.PackageName ??
                          AnsiConsole.Ask<string>("[dim]Package name[/] (e.g. [cyan]com.studio.tool[/]):");
        var outputPath = settings.OutputPath ?? ".";

        GitLogic.TryGetGitConfig(out var gitName, out _);
        var defaultAuthor = !string.IsNullOrEmpty(gitName) ? gitName : "YourCompany";
        var defaultDisplay = !string.IsNullOrEmpty(settings.DisplayName)
            ? settings.DisplayName
            : GetDisplayNameFromPackage(packageName);

        AnsiConsole.MarkupLine("[bold]Package Information[/]\n");
        var displayName = settings.DisplayName ?? AnsiConsole.Ask("[dim]Display name[/]:", defaultDisplay);
        var description = settings.Description ??
                          AnsiConsole.Ask<string>("[dim]Description[/]:", $"Generated by {displayName}");
        var authorName = settings.AuthorName ?? AnsiConsole.Ask("[dim]Author/Company name[/]:", defaultAuthor);
        ...
        if (!TemplateGeneratorLogic.TryGenerateFromTemplate(templatePath, fullOutputPath, packageName, displayName,
                description, authorName, unityVersion, out var fileCount))
        {
    ...
    {
        [CommandArgument(0, "[name]")] public string? PackageName { get; init; }

        [CommandOption("-d|--display")] public string? DisplayName { get; init; }

        [CommandOption("-o|--output")] public string? OutputPath { get; init; }

        [CommandOption("-t|--template")] public string? TemplateName { get; init; }

        [CommandOption("-l|--link")] public bool LinkToUnity { get; init; }

        [CommandOption("--desc|--description")]
        public string? Description { get; init; }

        [CommandOption("--author")] public string? AuthorName { get; init; }

        [CommandOption("--unity")] public string? UnityVersion { get; init; }
    }
}